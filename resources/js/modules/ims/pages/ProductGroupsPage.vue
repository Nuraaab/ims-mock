<template>
    <DashboardLayout>
        <section class="mb-6 flex items-center justify-between gap-3">
            <div>
                <h2 class="ui-text text-2xl font-semibold tracking-tight">Product Groups</h2>
                <p class="ui-muted mt-1 text-sm">Manage product groups for inventory categorization.</p>
            </div>
            <AppButton :disabled="loading" @click="openCreate">Create Product Group</AppButton>
        </section>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr v-if="!items.length && !loading">
                        <td class="ui-muted px-4 py-4" colspan="4">No product groups found.</td>
                    </tr>
                </tbody>
            </table>
        </section>
        ...
        <AppModal :open="modalOpen" :title="editingId ? 'Edit Product Group' : 'Create Product Group'" @close="closeModal">
            <form class="grid grid-cols-1 gap-3" @submit.prevent="submit">
                ...
                <div>
                    ...
                    <textarea v-model.trim="form.description" class="ui-input min-h-24" :disabled="saving" />
                </div>
                <div>
                    <label class="ui-label">Branch (Optional)</label>
...
import { useProductGroups } from "@ims/composables/useProductGroups";
import { useNotifier } from "@/shared/composables/useNotifier";

const { items: branches, fetchBranches } = useBranches();
const { items, loading, saving, error, validationErrors, fetchProductGroups, createProductGroup, updateProductGroup, deleteProductGroup, resetProductGroupFeedback } = useProductGroups();
...
async function submit() {
    ...
    try {
        ...
        notifySuccess(data?.message || (editingId.value ? "Product group updated successfully." : "Product group created successfully."));
    } catch (apiError) {
        notifyError(getErrorMessage(apiError, "Failed to save product group."));
        return;
    }
...
async function removeItem(id) {
    const confirmed = await confirmDelete({
        ...
        text: "This will permanently remove this product group.",
    });

    if (!confirmed) return;

    try {
        const data = await deleteProductGroup(id);
        notifySuccess(data?.message || "Product group deleted successfully.");
    } catch (apiError) {
        notifyError(getErrorMessage(apiError, "Failed to delete product group."));
    }
}
